...
RUN echo "Fetching YGG-API gist hash: ${GIST_HASH}" && \
    ...
    cp /Clemv95/ygg-api/ygg-api-download.yml /app/Definitions/ && \
    rm -rf /Clemv95

# Download ygege indexer
RUN git clone https://github.com/UwUDev/ygege.git /tmp/ygege && \
    cp /tmp/ygege/ygege.yml /app/Definitions/ && \
    rm -rf /tmp/ygege

# Download lacale indexer
RUN wget https://raw.githubusercontent.com/JigSawFr/lacale-prowlarr-indexer/refs/heads/main/lacale-api.yml \
    -O /app/Definitions/lacale-api.yml
